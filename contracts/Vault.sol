...
import "./lib/ControllableInit.sol";
...
contract Vault is ERC20Upgradeable, IVault, IUpgradeSource, ControllableInit, VaultStorage {
  ...
  function initializeVault(
    ...
    __ERC20_init(
      string(abi.encodePacked("Polycrab ", ERC20Upgradeable(_underlying).symbol())),
      string(abi.encodePacked("crab", ERC20Upgradeable(_underlying).symbol()))
    );
  ...
  modifier defense() {
    require(
      ...
      "Contract not whitelisted"  // check to see if it is on our whitelist.
    );
    _;
  }
